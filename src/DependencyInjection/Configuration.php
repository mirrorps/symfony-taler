        ...
        $rootNode
            ->children()
                ...
                ->variableNode('http_client')
                    ->defaultNull()
                    ->info('PSR-18 HTTP client service id, or null to auto-use Symfony psr18.http_client when framework.http_client is enabled.')
                    ->validate()
                        ->ifTrue(static fn (mixed $value): bool => $value !== null && !\is_string($value))
                        ->thenInvalid('The http_client option must be a service id string or null.')
                    ->end()
                ->end()
